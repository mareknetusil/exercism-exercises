...
def check_drinks(drink_name: str, drink_ingredients: list[str]) -> str:
    ...
    """
    drink_type = 'Mocktail' if ALCOHOLS.isdisjoint(drink_ingredients) \
        else 'Cocktail'
...
def tag_special_ingredients(
        dish: tuple[str, tp.Iterable[str]]
) -> tuple[str, list[str]]:
...
def compile_ingredients(dishes: list[set[str]]) -> set[str]:
    ...
    """
    # return ft.reduce(lambda s1, s2: s1 | s2, dishes, set())
...
def singleton_ingredients(dishes: list[set[str]], intersection: set[str]) -> set[str]:
    ...
    # return ft.reduce(
    #     lambda s1, s2: s1 | s2,
    #     (dish - intersection for dish in dishes)
    # )
